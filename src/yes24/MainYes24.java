...
//		}
		...
		List<Yes24Dto> list = new ArrayList<Yes24Dto>();

		System.out.println("--");

		List<String> tmpTitle = new ArrayList<>();
		...
		List<String> tmpPrice = new ArrayList<>();
		for (Element e : elem.select("strong")) {
			if (e.className().equals("txt_num")) {
				tmpPrice.add(e.text());

			}
		}

		List<String> tmpAuthor= new ArrayList<>();
		for (Element e : elem.select("span")) {
			if (e.className().equals("authPub info_auth")) {
				tmpAuthor.add(e.text());
		...
		for (Element e : elem.select("span")) {
			...
			if (e.className().equals("authPub info_date")) {
				num = num + 1;
				dto.setTitle(tmpTitle.get(num));
				dto.setPrice(tmpPrice.get(num));
				dto.setAuthor(tmpAuthor.get(num));
				dto.setDate(e.text());
				list.add(dto);
			}
		}
